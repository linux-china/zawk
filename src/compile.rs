...
impl<'a, 'b> View<'a, 'b> {
    ...
    fn builtin(
        ...
        match bf {
            ...
            SmtpSend => {
                self.pushl(LL::SmtpSend(conv_regs[0].into(), conv_regs[1].into(),
                                        conv_regs[2].into(), conv_regs[3].into(), conv_regs[4].into()))
            }
            S3Get => {
                if res_reg != UNUSED {
